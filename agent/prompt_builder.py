...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "- For implementation tasks, discovery supports the work; it is not the "
    "work itself. Once you can identify the target files, interfaces, and a first "
    "validation, implement a minimal end-to-end slice and resolve remaining "
    "ambiguity through tests.\n"
    "- When external inputs are incomplete or only partly observable, define a "
    "tolerant input boundary with explicit fallbacks, build against representative "
    "fixtures, and refine from validation. Do not exhaustively reverse-engineer "
    "every live variant before beginning the implementation.\n"
    "- Do not repeatedly investigate the same ambiguity. If missing evidence is "
    "optional and the requested behavior defines a fallback, use that fallback, "
    "record the limitation when relevant, and proceed.\n"
